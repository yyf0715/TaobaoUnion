    ...
    public class InnerHolder extends RecyclerView.ViewHolder {
        ...
        public void setData(ILinearItemInfo dataBean) {
            ...
            if(!TextUtils.isEmpty(cover)) {
                String coverPath = UrlUtils.getCoverPath(dataBean.getCover());//获取正确的cover地址
                Glide.with(context).load(coverPath).into(this.coverIv);//加载图片并绑定
            } else {
                coverIv.setImageResource(R.mipmap.ic_launcher);//没有就加载默认图片
            }
            ...
            offPriseTv.setText(String.format(context.getString(R.string.text_goods_off_prise),couponAmount));//省下多少元
